// Database initialization script for QuickCourt Booking System
// Run this script to set up the initial database structure for courts, time slots, and bookings
...
async function initBookingDatabase() {
  ...
  try {
    ...
    await db.collection('timeSlots').createIndex({ available: 1 })
     ...
     // Create a compound index for preventing double bookings
     // This ensures no two bookings can have the same court, date, and overlapping time
    ...
    console.log('Collections created: courts, timeSlots, bookings')
...
initBookingDatabase().catch(console.error)
